...
const SingUpEmailValidationForm = () => {
  ...
  const onSubmit = useCallback(async () => {
    ...
    // if (response.exists) {
    //   navigate(routes.passwordInstructions.path, { replace: true, state: { email: data.email } });
    // } else {
    //   setAlert({ show: true, text: "This email address does not exist!", severity: "error" });
    // }
    navigate(routes.singUp.path);
  });
      ...
      <FormManager initialState={values} onSubmit={onSubmit}>
        <div className={styles.form}>
          ...
          <FormManager.FormControl>
            <Typography className={styles.title} text={t("singUpEmail.title")} variant="h3" />
            <Typography
              className={styles.subtitle}
              text={t("singUpEmail.subtitle")}
              variant="body"
            />
            <Typography className={styles.label} text={t("singUpEmail.email")} variant="body" />
            <FormManager.FormField
              Field={TextField}
              className={styles.field}
              name="email"
              placeholder={t("singUpEmail.placeholder")}
              type="email"
            />
          </FormManager.FormControl>
          <div className={styles.buttonContainer}>
            <FormManager.FormSubmit
              ...
              text={t("singUpEmail.submit")}
              variant="contained"
            />
            <Button
              className={styles.button}
              onClick={handleOnClickReturn}
              text={t("singUpEmail.return")}
              variant="contained"
            />
          </div>
        </div>
...
export default SingUpEmailValidationForm;
